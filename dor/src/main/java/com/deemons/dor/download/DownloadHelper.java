...
import com.deemons.dor.download.temporary.TemporaryRecordTable;
...
import static com.deemons.dor.download.constant.Constant.DOWNLOAD_URL_EXISTS;
...
public class DownloadHelper {
    ...
    public DownloadHelper(Context context,DownloadApi api) {
        downloadApi = api;
    ...
    public File[] getFiles(String url) {
        ...
        if (record == null) {
            ...
            return ResponesUtils.getFiles(record.saveName, record.savePath);
        }
    }
     ...
     */
    public Observable<Status> downloadDispatcher(final DownloadBean bean) {
        return Observable.just(1)
                ...
                .flatMap(new Function<Integer, ObservableSource<Task>>() {
                    @Override
                    public ObservableSource<Task> apply(Integer integer) throws Exception {
                        return getDownloadType(bean.url);
                    }
                })
                .flatMap(new Function<Task, ObservableSource<Status>>() {
                    @Override
                    public ObservableSource<Status> apply(Task type) throws Exception {
                ...
                .doFinally(new Action() {
                    ...
                    public void run() throws Exception {
                        recordTable.delete(bean.url);
                    }
                });
    }

    private ObservableSource<Status> download(Task downloadType)
    ...
    private void addTempRecord(DownloadBean bean) {
        if (recordTable.contain(bean.url)) {
            throw new IllegalArgumentException(formatStr(DOWNLOAD_URL_EXISTS, bean.url));
        }
        recordTable.add(bean.url, new TemporaryRecord(bean));
    }
     ...
     * @return download type
     */
    private Observable<Task> getDownloadType(final String url) {
        return Observable.just(1)
                ...
                .flatMap(new Function<Object, ObservableSource<Task>>() {
                    @Override
                    public ObservableSource<Task> apply(Object o) throws Exception {
     ...
     * @return Download Type
     */
    private Observable<Task> nonExistsType(final String url) {
        return Observable.just(1)
                .flatMap(new Function<Integer, ObservableSource<Task>>() {
                    @Override
                    public ObservableSource<Task> apply(Integer integer)
                            throws Exception {
     ...
     * @return Download Type
     */
    private Observable<Task> existsType(final String url) {
        return Observable.just(1)
                ...
                .flatMap(new Function<Object, ObservableSource<Task>>() {
                    @Override
                    public ObservableSource<Task> apply(Object o)
                            throws Exception {
